...
export default function RouteFeedback() {
    ...
    try {
      ...
      const impression = await fetch(`${API_BASE}/api/route-impressions`, {
        ...
        body: JSON.stringify({
          ...
          rank: Math.max(1, recommendations.findIndex((item) => item.route.id === selectedRoute.route.id) + 1),
  ...
  return (
    <section className="route-feedback" aria-label="경로 이용 후기">
      ...
      <fieldset><legend>다시 이용하시겠어요?</legend><button type="button" className={`chip ${wouldReuse === true ? 'chip--active' : ''}`} onClick={() => setWouldReuse(true)}>예</button><button type="button" className={`chip ${wouldReuse === false ? 'chip--active' : ''}`} onClick={() => setWouldReuse(false)}>아니요</button></fieldset>
      <fieldset><legend>시설물·경로 정보가 실제와 같았나요?</legend><button type="button" className={`chip ${informationAccurate === true ? 'chip--active' : ''}`} onClick={() => setInformationAccurate(true)}>같았어요</button><button type="button" className={`chip ${informationAccurate === false ? 'chip--active' : ''}`} onClick={() => setInformationAccurate(false)}>달랐어요</button></fieldset>
      <label>추가 의견 <textarea value={comment} maxLength={2000} onChange={(event) => setComment(event.target.value)} /></label>
