...
`20260724_0003`은 기존 후기를 그대로 유지하면서 혼잡, 환승 안내·정보,
...
후기의 혼잡·환승 안내·교통약자 시설 불편도는 실제 이용자가 선택적으로
입력한 직접 관측값으로만 저장합니다. 2023~2025 만족도 조사 점수를
경로별 값으로 추론하지 않으며, 이 세 항목은 현재 온라인 개인화 target과
전역 후보 relevance를 자동 변경하지 않습니다. 향후 분석에서도 기존
`trainingConsent`와 관리자 수동 검토 경계를 그대로 적용합니다.
