...
if os.getenv("RUN_LIVE_TESTS") != "1":
    pytest.skip("RUN_LIVE_TESTS=1인 명시적 외부 연동 검증에서만 실행합니다.", allow_module_level=True)
if not settings.ODSAY_API_KEY or settings.ODSAY_API_KEY.startswith("YOUR_"):
    pytest.skip("실제 ODSAY_API_KEY가 필요합니다.", allow_module_level=True)
...
def test_odsay_returns_candidates():
    ...
    for i, c in enumerate(candidates):
        ...
        print(f"    환승: {info.get('transferCount', '미확인')}회")
        print(f"    도보: {info.get('totalWalk', '미확인')}m")
...
def test_parse_api_features():
    ...
    assert feats["is_low_floor_bus"] in (None, False, True)
    assert feats["elevator_ratio"] is None or 0.0 <= feats["elevator_ratio"] <= 1.0
