...
`transfer_count`와 `minimize_transfers_burden`에는 감소 방향의 XGBoost
단조 제약을 적용한다. 다른 피처가 모두 같을 때 환승이 늘어난 후보의 모델
점수가 올라가서는 안 된다. 이 제약은 환승 경로를 무조건 배제하지 않으며,
소요시간·도보거리 등 실제로 다른 피처와의 비교는 학습 순위화에 남긴다.
고정 OD-group 홀드아웃과 실제 회귀 OD를 함께 재검증한 학습 설정은 artifact의
`ranker_parameters`에 기록해 동일 모델을 재현할 수 있게 한다.
