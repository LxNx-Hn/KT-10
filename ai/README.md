...
휠체어 설정은 일반 경로와 분리됩니다. 수집기는 TMAP 보행자 경로의
`searchOption=30`을 보내고, 공급자가 계단 제외로 확인한 보행 구간만
백엔드 휠체어 필터를 통과합니다. 응답의 `turnType=128/129` 안내점 또는
`facilityType=19/20` 시설 구간이 있을 때만 물리 경사로 좌표를 공개합니다.
