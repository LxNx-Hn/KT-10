"""고정된 실제 후보 스냅샷에서 초기 평가용 빈 평가표를 만든다."""
...
def prepare(
    *,
    ...
    if (
        not evaluation_run_id.strip()
        or not evaluation_source.strip()
        or not rubric_version.strip()
    ):
        raise ValueError(
            "evaluation_run_id, evaluation_source, rubric_version은 "
            "비어 있을 수 없습니다."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="실제 후보 스냅샷용 초기 평가표 생성"
    )
